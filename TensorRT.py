...
def main(onnx_model_file, batch_size):
    ...
    with build_engine_onnx(TRT_LOGGER, onnx_model_file, batch_size) as engine:
        ...
        with engine.create_execution_context() as context:
            ...
            for imgs, targets in val_loader:
                ...
                tot += 64
            print(f"Inference Time: {trt_time:.5f}")
            print(trt_time)
            print(f"Validation Accuracy: {correct/tot:.3f}")
            #print(np.sum(np.argmax(np.reshape(np.array(trt_outputs), (batch_size,10)), axis = 1)-trgts))

if __name__ =='__main__': 
    main(args.load_from, args.batch_size)